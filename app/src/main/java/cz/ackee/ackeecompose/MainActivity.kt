                ...
                AckeeComposeTheme {
                    ...
                    Surface(color = MaterialTheme.colors.background) {
                        NavHost(navController = navController, startDestination = Destination.CharacterList.route) {
                            composable(Destination.CharacterList) { CharacterList(viewModel.charactersPagedList.collectAsLazyPagingItems()) }
                            composable(Destination.CharacterDetail) {
                                ...
                                val character = navController.previousBackStackEntry?.arguments?.getParcelable<Character>(Destination.CharacterDetail.Argument.Character.name)
                                if (character != null) {
